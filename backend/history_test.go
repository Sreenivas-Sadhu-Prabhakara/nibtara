...
func TestPostgresStore_RoundTrip(t *testing.T) {
	...
	in, _ := json.Marshal(Due{Customer: "Kumar", Amount: 1000, SinceDate: "2026-05-01"})
	saved, err := store.Save(Record{Input: in, Headline: 1000, Label: "Kumar"})
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	if saved.ID == 0 {
	...
	if len(items) == 0 || items[0].ID != saved.ID || items[0].Label != "Kumar" {
